...
## 概要
現在のAIアシスト型ワークフローを強化し、以下の改善を行います：
1. 左右ページの違いに対応した最適なPDF処理の実現
2. プレビュー画像を一枚ずつ表示する機能の追加（API error回避）
3. ページタイプ情報の明示による判断精度の向上

ユーザーが開き方を指定し、AIが左右ページそれぞれに最適な余白設定を提案、それに基づいてクロップを実行する流れを実現します。

## 現状の課題
1. 左右ページの違いを考慮していないため、すべてのページに同一の余白設定が適用され、最適な結果が得られない
2. 複数のプレビュー画像を一度に表示するとAPI errorが発生することがある
3. ページタイプ（左/右）の情報がAIに明示的に伝わらない
...
#### 2.1 一枚ずつ表示する機能の追加
プレビュー画像を一枚ずつ表示するオプションを追加し、デフォルト設定とする。これによりAPI errorを回避する。
...
@cli.option(
    ...
    is_flag=True,
    default=True,
    help='プレビュー画像を一枚ずつ表示（デフォルト：有効）'
)
@cli.option(
    '--multi-view',
    is_flag=True,
    help='プレビュー画像を複数枚一度に表示（注：API errorの可能性あり）'
)
```
...
```python
# プレビュー処理を修正
if single_view and not multi_view:
    # 一枚ずつ表示モード
    for i, file_path in enumerate(exported_files):
        click.echo(f"プレビュー画像 {i+1}/{len(exported_files)}: {file_path}")
        click.echo("AIに表示したら次の画像に進むには Enter キーを押してください...")
        input()  # ユーザー入力待ち
else:
    # 複数枚表示モード（従来通り、すべての画像パスを一度に表示）
    click.echo("プレビュー画像を出力しました！")
...
def preview(input_pdf, output_dir, pages, num_pages, dpi, binding, first_page_right, single_view, multi_view):
    """
    ...
    デフォルトでは一枚ずつ表示モードになっており、AIのAPI errorを回避できます。
...
text = f"ページ {page_num} ({page_type_jp}) - {binding_jp}開き"
...
READMEに左右ページ対応とプレビュー一枚ずつ表示のワークフローを追加：
...
1. 開き方を指定してプレビュー画像を生成
   ...
   pdf-kindle preview input/book.pdf --binding vertical
   ```
   画像が一枚ずつ表示されるので、AIに一枚ずつ解析してもらいます。
   （複数画像を同時表示する場合は `--multi-view` オプションを追加）
...
.rooファイルに左右ページ対応の質問と回答例、および一枚ずつ処理するルールを追加：

```
## 左右ページ別の余白除去

【重要】プレビュー画像は一枚ずつ表示し、一枚ずつ処理してください。複数画像を一度に処理しようとするとエラーが発生する場合があります。

【質問例】
「このPDFページ画像を見て、左右ページ別の最適なクロップ位置を%で教えてください。これは[左/右]ページで、[縦/横]開きの本です。」

【回答例】
PDFのプレビュー画像を分析しました。これは右ページで、縦開きの書籍PDFです。

左右ページそれぞれの最適なクロップ設定を提案します：
...
1. プレビュー画像の一枚ずつ表示機能 - API errorの解消が最優先
2. ページタイプ（左/右）判定と表示機能
3. クロップコマンドの左右ページ別設定対応
4. ドキュメントの更新
5. 自動検出機能の強化（オプション）
...
1. **API errorの回避**
   - 一枚ずつ表示モードでAI処理時のエラーを解消
   - より安定したワークフロー

2. **ページに応じた最適化**
   - 左右ページそれぞれに最適な余白設定を適用
   - 綴じ側と外側で異なる余白特性に対応
